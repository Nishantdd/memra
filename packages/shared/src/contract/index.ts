...
export const authContract = {
  ...
  session: oc.route({ method: "GET", path: "/auth/session" }).output(
    z.object({ authenticated: z.boolean(), expiresAt: z.number().int().nullable() }),
  ),
  changePassword: oc
    .route({ method: "POST", path: "/auth/password" })
    .errors({ UNAUTHORIZED: {}, BAD_REQUEST: { data: z.object({ reason: z.string() }) } })
    .input(z.object({ current: z.string().min(1), next: z.string().min(LIMITS.passwordMin).max(1024) }))
    .output(Ok),
...
export const notesContract = {
  list: oc
    ...
    .input(z.object({ folderId: Uuid.nullable().optional(), limit: z.number().int().min(1).max(200).default(50), offset: z.number().int().min(0).default(0) }))
  ...
  update: oc
    ...
    .input(NotePatch)
    .output(Note),
  delete: oc.route({ method: "DELETE", path: "/notes/{id}" }).errors(notFound).input(ById).output(Note),
  restore: oc.route({ method: "POST", path: "/notes/{id}/restore" }).errors(notFound).input(ById).output(Note),
};
...
export const foldersContract = {
  ...
  rename: oc
    ...
    .output(Folder),
  delete: oc.route({ method: "DELETE", path: "/folders/{id}" }).errors(notFound).input(ById).output(Folder),
  reorder: oc
...
export const tagsContract = {
  ...
  rename: oc
    ...
    .output(Tag),
  delete: oc.route({ method: "DELETE", path: "/tags/{id}" }).errors(notFound).input(ById).output(Tag),
};
...
export const syncContract = {
  pull: oc
    ...
    .input(z.object({ cursor: z.number().int().nonnegative().default(0), limit: z.number().int().min(1).max(LIMITS.syncPageMax).default(LIMITS.syncPageMax) }))
    .output(SyncPage),
