...
const SELECT =
  "SELECT id, name, sort_order, created_at, updated_at, deleted_at, server_seq FROM folders";
...
export class FoldersRepo {
  ...
  rename(id: string, name: string): { folder: Folder; noteIds: string[] } {
    return this.#db.transaction(() => {
      ...
      this.#db.run(
        "UPDATE folders SET name = ?, updated_at = ?, server_seq = ? WHERE id = ?",
        name,
        now(),
        nextSeq(this.#db),
        id,
  ...
  softDelete(id: string): { folder: Folder; noteIds: string[] } {
    return this.#db.transaction(() => {
      ...
      for (const noteId of noteIds) {
        this.#db.run(
          "UPDATE notes SET folder_id = NULL, updated_at = ?, server_seq = ? WHERE id = ?",
          t,
          nextSeq(this.#db),
          noteId,
        );
      ...
      this.#db.run(
        "UPDATE folders SET deleted_at = ?, updated_at = ?, server_seq = ? WHERE id = ?",
        t,
        t,
        nextSeq(this.#db),
        id,
  ...
  reorder(ids: string[]): Folder[] {
    return this.#db.transaction(() => {
      ...
      ids.forEach((id, i) => {
        this.#db.run(
          "UPDATE folders SET sort_order = ?, updated_at = ?, server_seq = ? WHERE id = ?",
          i,
          t,
          nextSeq(this.#db),
          id,
