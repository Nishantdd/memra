...
export class NotesRepo {
  ...
  list(folderId: string | null | undefined, limit: number, offset: number): { items: Note[]; total: number } {
    const where = folderId === undefined ? "n.deleted_at IS NULL" : "n.deleted_at IS NULL AND n.folder_id IS ?";
    const params = folderId === undefined ? [] : [folderId];
    const items = this.db
      .all<NoteRow>(`${SELECT} WHERE ${where} ORDER BY n.pinned DESC, n.updated_at DESC LIMIT ? OFFSET ?`, ...params, limit, offset)
      .map(toNote);
    const total = this.db.get<{ c: number }>(`SELECT count(*) AS c FROM notes n WHERE ${where}`, ...params)!.c;
    return { items, total };
  }
    ...
    return this.db.transaction(() => {
      ...
      this.db.run(
        `INSERT INTO notes(id, folder_id, title, body_md, body_plain, display_title, excerpt, color, pinned,
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 0, ?, ?, ?, NULL, ?)`,
        id, input.folderId, input.title, input.bodyMd, d.bodyPlain, d.displayTitle, d.excerpt, input.color,
        input.pinned ? 1 : 0, input.sourceFilename, t, t, nextSeq(this.db),
      );
  ...
  update(patch: NotePatch): Note {
    return this.db.transaction(() => {
      ...
      this.db.run(
        ...
        title, bodyMd, d.bodyPlain, d.displayTitle, d.excerpt,
        patch.color ?? current.color, (patch.pinned ?? current.pinned) ? 1 : 0,
        version, now(), nextSeq(this.db), patch.id,
      );
      if (patch.tagIds) this.setTags(patch.id, patch.tagIds);
      this.refreshFts(patch.id);
      const tagsChanged = patch.tagIds !== undefined && patch.tagIds.slice().sort().join() !== current.tagIds.join();
      ...
      else this.db.run("UPDATE notes SET indexed_version = ? WHERE id = ? AND indexed_version = ?", version, patch.id, current.version);
  ...
  softDelete(id: string): Note {
    return this.db.transaction(() => {
      ...
      const t = now();
      this.db.run("UPDATE notes SET deleted_at = ?, updated_at = ?, server_seq = ? WHERE id = ?", t, t, nextSeq(this.db), id);
      this.db.run("DELETE FROM notes_fts WHERE rowid = (SELECT rowid FROM notes WHERE id = ?)", id);
  ...
  restore(id: string): Note {
    return this.db.transaction(() => {
      ...
      this.db.run("UPDATE notes SET deleted_at = NULL, updated_at = ?, server_seq = ? WHERE id = ?", now(), nextSeq(this.db), id);
      this.refreshFts(id);
  ...
  refreshFts(noteId: string): void {
    const row = this.db.get<{ rowid: number; display_title: string; body_plain: string; deleted_at: number | null; folder: string | null; tags: string | null }>(
    ...
    this.db.run(
      ...
      BigInt(row.rowid), row.display_title, row.tags ?? "", row.body_plain, row.folder ?? "",
    );
  }
