...
export class TagsRepo {
  ...
  list(): Tag[] {
    return this.#db
      .all<TagRow>(`${SELECT} WHERE deleted_at IS NULL ORDER BY lower(name)`)
      .map(toTag);
  }
  ...
  rename(id: string, name: string): { tag: Tag; noteIds: string[] } {
    return this.#db.transaction(() => {
      ...
      this.#db.run(
        "UPDATE tags SET name = ?, updated_at = ?, server_seq = ? WHERE id = ?",
        name,
        now(),
        nextSeq(this.#db),
        id,
  ...
  softDelete(id: string): { tag: Tag; noteIds: string[] } {
    return this.#db.transaction(() => {
      ...
      for (const noteId of noteIds) {
        this.#db.run(
          "UPDATE notes SET updated_at = ?, server_seq = ? WHERE id = ?",
          t,
          nextSeq(this.#db),
          noteId,
        );
      ...
      this.#db.run(
        "UPDATE tags SET deleted_at = ?, updated_at = ?, server_seq = ? WHERE id = ?",
        t,
        t,
        nextSeq(this.#db),
        id,
